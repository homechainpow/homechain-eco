...
async fn start_mining(
    ...
    // Spawn mining worker — runs until cancelled or error
    ...
    Ok("Miner started gracefully".to_string())
}
...
async fn stop_mining(state: State<'_, MinerState>) -> Result<String, String> {
    ...
    if let Some(tx) = tx_guard.take() {
        let _ = tx.send(true); // Signal the watch channel: "please stop"
    }
    drop(tx_guard);
